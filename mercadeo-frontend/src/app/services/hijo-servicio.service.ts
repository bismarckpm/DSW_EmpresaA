...
export class HijoServicioService {
  ...
  createHijo(hijos: Hijo[]) {
    ...
    return this.httpClient.post(`http://45.76.60.252:8080/mercadeo-backend/api/hijo/addHijo`, hijos)
    .subscribe(
      response => {
  ...
  getHijos(id: number): Observable<any> {
    return this.httpClient.get(`http://45.76.60.252:8080/mercadeo-backend/api/hijo/HijosUsuario/${id}`);
  }

  setHijos( hijos: Hijo[]) {
    return this.httpClient.put(`http://45.76.60.252:8080/mercadeo-backend/api/hijo/updateHijo`, hijos)
    .subscribe(
